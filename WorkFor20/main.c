...
Student stu[1020];int cnt;

void swap(Student* a,Student* b){Student temp;temp=*a;*a=*b;*b=temp;}
...
void add(char first_name[100],char last_name[100],char id[20],int score)
{
  cnt++;
  ...
  sprintf(stu[cnt].GPA,"%s",assgpa(score));
  id[4]=0;
  if( strcmp(stu[cnt].id,"2022") )
    stu[cnt].relearn=true;
}

void Delete(char id[20])
{
  for(int i=1;i<=cnt;i++){
...
void mergesort(int l,int r)
{
  ...
  int k= 0;
  while(i<=mid && j<=r ){
    if(stu[i].score_c>stu[j].score_c)temp[k++]=stu[i++];
    else temp[k++]=stu[j++];
  }
...
void Sort_by_score()
{
  ...
  printf("%-14s %-12s %-12s %-3s %-3s %-3s %-4s\n","学号","first_name","last_name","重修","C语言成绩","GPA","班级排名");
  for(int i=1;i<=cnt;i++){
    printf("%-12s %-12s %-12s %-4s %-3d %9s %-4d\n",stu[i].id,stu[i].first_name,stu[i].last_name,stu[i].relearn?"yes":"no",stu[i].score_c,stu[i].GPA,stu[i].Rank);
  }
}
...
{
  static char pvkey[3020];int pv=0;
  char temp[4];pv=0;
  for(int i=1;i<=cnt;i++){
    // itoa(stu[i].score_c,temp,10); //gcc don't support itoa
...
{
  if(PVkey==NULL)return "请先加密再解密\n";
  static char pckey[3020];int pc=0;
  int len=strlen(str);
  for(int i=0;i<len;i++){
...
int main()
{
  ...
  printf("okay, data upload finished. what do you what to do next? You can enter a number to tel1 me.\n");
  menu();
  while(scanf("%d",&op),op){
    switch (op){
    case 1:{
      ...
      int score;
      printf("please input the first_name,last_name,id and the score\n");
